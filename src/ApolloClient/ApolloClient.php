...
namespace Mrstock\Servicesdk\ApolloClient;
...
class ApolloClient
{
     ...
     * @return mixed
     */
    public function start_back($callback = null)
    {
        $ch = curl_init();
    ...
    public function start()
    {
        ...
        $params['notifications'] = json_encode(array_values($this->notifications));
        $query = http_build_query($params);
        $response = HttpRequest::query($base_url . $query);
        $apollo = [];
        if (!empty($response)) {
            $change_list = [];
            foreach ($response as $r) {
                if ($r['notificationId'] != $this->notifications[$r['namespaceName']]['notificationId']) {
                    ...
                    $query_args['ip'] = $this->clientIp;
                    $request = [];
                    $config_file = $this->getConfigFile($r['namespaceName']);
                    $request_url = $base_url . $r['namespaceName'];
                    $query_args['releaseKey'] = $this->_getReleaseKey($config_file);
                    $query_string = '?' . http_build_query($query_args);
                    $request_url .= $query_string;
                    $result = HttpRequest::query($request_url);
                    if (!empty($result)) {
                        $apollo = array_merge($apollo, $result['configurations']);
                    }
                }
            }
            include ENV_TPL;
            $a = file_put_contents(ENV_FILE, $env);
            echo $a . PHP_EOL;
        }
